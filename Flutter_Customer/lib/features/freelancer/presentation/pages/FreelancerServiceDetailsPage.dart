...
class FreelancerServiceDetailsPage extends StatefulWidget {
  ...
  const FreelancerServiceDetailsPage({super.key, this.service});

  @override
  State<FreelancerServiceDetailsPage> createState() => _FreelancerServiceDetailsPageState();
}

class _FreelancerServiceDetailsPageState extends State<FreelancerServiceDetailsPage> {
  ...
  Widget _buildHeaderSection(BuildContext context, Map<String, dynamic>? service) {
    final category = _asString(service?['category'], fallback: 'Service');
  ...
  Widget _buildServiceCard(Map<String, dynamic> item) {
    ...
    return GestureDetector(
      onTap: () {
        // Navigate to details page for this service
        // Since we are already on details page, we can push a new one
        context.push('/freelancer-service-details', extra: item);
      },
      child: Container(
        width: 220,
        decoration: BoxDecoration(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              return _buildServiceCard(_recentlyViewed[index]);
            },
          ),
        ),
      ],
