        ...
        @if($selectedConversation)
            ...
            <div class="p-4 bg-white border-t border-slate-200 shrink-0">
                <form id="chat-form" action="{{ route('customer.chat.store') }}" method="POST" enctype="multipart/form-data" class="relative">
                    ...
                    <!-- File Preview -->
                    <div id="attachment-preview" class="hidden px-4 py-2 bg-slate-50 border-x border-t border-slate-300 rounded-t-xl flex items-center justify-between">
                        <div class="flex items-center gap-2 text-sm text-slate-600">
                            <i class="fas fa-file text-primary-500"></i>
                            <span id="file-name" class="truncate max-w-[200px]">filename.jpg</span>
                        </div>
                        <button type="button" onclick="removeAttachment()" class="text-slate-400 hover:text-red-500 transition-colors">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>

                    <div class="bg-white border border-slate-300 rounded-xl focus-within:ring-1 focus-within:ring-slate-400 focus-within:border-slate-400 transition-all shadow-sm relative z-10" id="message-container">
                        ...
                        <div class="flex justify-between items-center px-2 pb-2">
                            <div class="flex items-center gap-0.5">
                                ...
                                <button type="button" onclick="document.getElementById('file-input').click()" class="p-2 text-slate-400 hover:text-slate-600 transition-colors rounded-full hover:bg-slate-100" title="Attach file">
...
<script>
    function handleFileSelect(input) {
        if (input.files && input.files[0]) {
            ...
            messageContainer.classList.remove('rounded-xl');
            messageContainer.classList.add('rounded-b-xl', 'rounded-t-none', 'border-t-0');
            
            // Remove required attribute from textarea since we have an attachment
    ...
    function removeAttachment() {
        ...
        // Reset message container styles
        const messageContainer = document.getElementById('message-container');
        messageContainer.classList.add('rounded-xl');
        messageContainer.classList.remove('rounded-b-xl', 'rounded-t-none', 'border-t-0');
        
        // Add required attribute back if textarea is empty
        ...
        if (messageInput) {
            messageInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    const hasAttachment = document.getElementById('file-input').files.length > 0;
