...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Manage supported languages</p>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 font-bold text-slate-700">{{ $language->name }}</td>
                        <td class="px-6 py-4 text-sm text-slate-500">{{ $language->code }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2.5 py-1 rounded-full text-xs font-bold {{ $language->is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-600' }}">
                                {{ $language->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right flex justify-end gap-2">
                            <button onclick="editLanguage({{ $language->id }}, '{{ $language->name }}', '{{ $language->code }}', {{ $language->is_active }})" class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-500 hover:bg-indigo-100 flex items-center justify-center transition-colors">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form action="{{ route('admin.languages.destroy', $language->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
...
<div id="createModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl transform transition-all scale-100">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-slate-800">Add Language</h3>
        ...
        <form action="{{ route('admin.languages.store') }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Code</label>
                    <input type="text" name="code" required maxlength="5" class="w-full px-4 py-2 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                </div>
...
<div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl transform transition-all scale-100">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-slate-800">Edit Language</h3>
        ...
        <form id="editForm" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">Code</label>
                    <input type="text" name="code" id="edit_code" required maxlength="5" class="w-full px-4 py-2 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                </div>
                <div class="flex items-center gap-2">
...
<script>
    function editLanguage(id, name, code, active) {
