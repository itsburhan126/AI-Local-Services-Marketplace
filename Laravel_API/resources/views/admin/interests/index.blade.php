...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <h3 class="text-xl font-bold text-slate-800 mb-6">Add New Interest</h3>
        <form action="{{ route('admin.interests.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6 md:flex-row md:items-end">
            @csrf
            <div class="flex-1">
    ...
    <div>
        <div class="flex justify-between items-center mb-6 px-2">
            <div>
                <h3 class="text-xl font-bold text-slate-800">Available Interests</h3>
        ...
        <div id="interests-grid" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            ...
            <div class="group relative bg-white rounded-2xl border border-slate-200 p-6 flex flex-col items-center gap-4 hover:shadow-xl hover:border-indigo-100 transition-all duration-300 cursor-move" data-id="{{ $interest->id }}">
                ...
                <div class="w-full pt-4 border-t border-slate-50 flex justify-center">
                    <form action="{{ route('admin.interests.destroy', $interest) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this interest?');">
                        @csrf
                        @method('DELETE')
            ...
            <div class="col-span-full py-16 text-center bg-white rounded-2xl border border-dashed border-slate-300">
                ...
                <h3 class="text-lg font-medium text-slate-900">No interests added yet</h3>
                <p class="mt-1 text-slate-500">Get started by adding your first interest above.</p>
            </div>
            @endforelse
        </div>
