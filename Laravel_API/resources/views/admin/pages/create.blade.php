    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                ...
                <p class="text-sm text-slate-500 mt-1">Add a new dynamic page to your website</p>
            </div>
            ...
            <form action="{{ route('admin.pages.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full px-4 py-2 rounded-lg border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-slate-700" required placeholder="e.g., Refund Policy">
                </div>

                <div>
                    <label for="content" class="block text-sm font-medium text-slate-700 mb-1">Content</label>
                    <div class="prose max-w-none">
                        <textarea name="content" id="summernote" class="w-full rounded-lg border border-slate-200">{{ old('content') }}</textarea>
                    </div>
                </div>
