...
<div class="content-transition">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 font-jakarta">Add Payout Method</h1>
            <p class="text-gray-500 mt-1">Create a new withdrawal option</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.payout-methods.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" 
                                      class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">{{ old('description') }}</textarea>
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Min Amount ($)</label>
                                <input type="number" name="min_amount" step="0.01" value="{{ old('min_amount', 0) }}" required 
                                       class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Max Amount ($)</label>
                                <input type="number" name="max_amount" step="0.01" value="{{ old('max_amount') }}" 
                                       class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="processing_time_days" value="{{ old('processing_time_days', 1) }}" required 
                                   class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all">
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-panel rounded-2xl p-6" x-data="fieldBuilder()">
