...
<div class="p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800">Create Guide</h1>
        <p class="text-slate-500 text-sm mt-1">Add a new guide or article</p>
    </div>
        ...
        <form action="{{ route('admin.guides.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" required>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="excerpt" rows="3" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('excerpt') }}</textarea>
                </div>

                <div class="col-span-2">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Content</label>
                    <textarea name="content" rows="10" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('content') }}</textarea>
                </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2 rounded-lg bg-indigo-600 text-white font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all">Create Guide</button>
            </div>
        </form>
    </div>
</div>
