...
@section('header', 'Freelancer Banners')

@section('content')
<div class="content-transition">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Freelancer Banners</h1>
            <p class="text-slate-500 text-sm mt-1">Manage promotional banners for the freelancer home page</p>
        </div>
        <a href="{{ route('admin.freelancers.index') }}" class="bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 px-4 py-2 rounded-xl shadow-sm flex items-center gap-2 text-sm font-bold transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="glass-panel rounded-2xl p-6">
                ...
                <form action="{{ route('admin.freelancers.banners.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="order" value="0" class="w-full px-4 py-2.5 rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 bg-white/50">
                        </div>
                        ...
                        <div class="aspect-video bg-slate-100 relative">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-4">
                                ...
                                <div class="flex justify-between items-center mt-1">
                                    <p class="text-white/80 text-xs">Order: {{ $banner->order }}</p>
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold {{ $banner->is_active ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-500/50 text-slate-300' }}">
                                        {{ $banner->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </div>
                            </div>
                            <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <form action="{{ route('admin.freelancers.banners.destroy', $banner->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this banner?');">
                                    @csrf
                                    @method('DELETE')
