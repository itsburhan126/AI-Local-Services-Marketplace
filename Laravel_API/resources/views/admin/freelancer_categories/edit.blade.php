...
@section('title', 'Edit Freelancer Category')

@section('content')
<div class="content-transition">
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('admin.freelancer-categories.index', ['parent_id' => $category->parent_id]) }}" class="w-10 h-10 rounded-xl bg-white border border-gray-100 flex items-center justify-center text-gray-500 hover:text-indigo-600 hover:shadow-md transition-all">
            <i class="fas fa-arrow-left"></i>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-800 font-jakarta">Edit Freelancer Category</h1>
            <p class="text-gray-500 mt-1">Update freelancer service category details</p>
        </div>
    </div>

    <div class="glass-panel rounded-2xl p-8 max-w-3xl">
        <form action="{{ route('admin.freelancer-categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <input type="number" name="commission_rate" step="0.01" min="0" max="100" value="{{ old('commission_rate', $category->commission_rate ?? 0) }}"
            ...
            <div class="space-y-2">
                ...
                @if($category->image)
                <div class="mb-4">
                    <img src="{{ $category->image }}" alt="Current Image" class="w-32 h-32 object-cover rounded-xl shadow-sm">
                </div>
                @endif
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-indigo-500 hover:bg-indigo-50/30 transition-all cursor-pointer relative group">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-sm text-gray-600 font-medium">Click to replace image</p>
            ...
            <div class="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div class="relative inline-block w-12 h-6 transition duration-200 ease-in-out">
                    <input type="checkbox" name="is_shown_in_footer" id="is_shown_in_footer" value="1" {{ old('is_shown_in_footer', $category->is_shown_in_footer ?? 0) ? 'checked' : '' }} class="peer absolute w-0 h-0 opacity-0">
