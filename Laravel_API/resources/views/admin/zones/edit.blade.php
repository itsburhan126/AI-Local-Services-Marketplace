...
<div class="content-transition">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 font-jakarta">Edit Zone</h1>
            <p class="text-gray-500 mt-1">Update service area details</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.zones.update', $zone->id) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <textarea name="coordinates" rows="6" required
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all bg-white/50 font-mono text-sm">{{ old('coordinates', $zone->coordinates) }}</textarea>
                @error('coordinates') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>

            <div class="pt-2">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="is_active" value="1" {{ $zone->is_active ? 'checked' : '' }} class="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300">
